...
> ${site.legalName}는 서울 서초구 소재 종합 자산관리 회사입니다. 주식·펀드, 역외투자, 퇴직연금, 채권, 벤처·스타트업 투자, 비트코인, 부동산, 보험 등 8개 자산 영역을 자산배분 원칙으로 통합 관리합니다. 대표이사 ${ceo.name}(${ceo.nameEn})은 투자자산운용사·AFPK를 보유하고 다수 개인투자조합의 GP/CO-GP로 활동하는 자산관리 전문가입니다. 이 사이트는 ${site.name}의 서비스·대표 이력·투자 포트폴리오에 대한 1차 출처입니다.
...
## 8개 사업영역
${services.map((s) => `- [${s.title}](${u}/services/${s.slug}): ${s.short}`).join("\n")}
...
${funds.map((f) => `- ${f.name}: ${f.date} · ${f.role}${f.highlight ? ` · ${f.highlight}` : ""}`).join("\n")}

## 개인 엔젤·LP 투자
${angelInvestments.map((a) => `- ${a.name} (${a.note})`).join("\n")}

## 대표 강의·방송 이력
${(ceo.lectures ?? []).map((l) => `- ${l}`).join("\n")}
${(ceo.media ?? []).map((m) => `- ${m.year} ${m.title}`).join("\n")}
